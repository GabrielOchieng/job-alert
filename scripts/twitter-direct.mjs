...
// async function runPivotScout() {
//   console.log("🕵️‍♂️ [Cipher Hunt] Starting Search Pivot...");

//   const query = `site:x.com "hiring" "frontend" "remote"`;
...
//       waitFor: 3000,
//     });
//     if (!scrapeResult.markdown) throw new Error("Scrape failed.");

//     const today = new Date().toDateString();
//     const prompt = `Context: Today is ${today}. Extract JSON: [{"title": "...", "company": "...", "url": "..."}] from markdown. Only posts from last 48h. Return ONLY raw JSON array.`;
...
//     console.log(`📡 Signals Found: ${jobs.length}`);

//     if (jobs.length === 0) {
//       console.log("ℹ️ No leads decoded. Terminating.");
//       return;
//     }
...
//       if (!existing) {
//         console.log(`➕ New lead detected! Inserting: ${job.title}`);
...
//               ...job,
//               location: "Remote (X-Signal)",
//               description: `Intercepted for ${job.title}`,
//             },
//           ])
//           .select()
//           .single();
...
//         }
//       } else {
//         console.log("⏭️ Lead already exists in Command Center. Skipping.");
//       }
//     }

//     if (newLeads.length > 0) {
//       console.log(`✅ ${newLeads.length} leads synced. Attempting Resend...`);

//       const { data, error } = await resend.emails.send({
//         from: "CipherHunt <onboarding@resend.dev>",
//         to: process.env.MY_EMAIL,
//         subject: `🎯 X-Signal: ${newLeads.length} New Leads`,
//         html: `<h1>Signals Decoded</h1>${newLeads.map((j) => `<p>${j.title} @ ${j.company}</p>`).join("")}`,
//       });

//       if (error) {
//         console.error("❌ RESEND REJECTED:", error.message);
//       } else {
//         console.log("📨 EMAIL SENT SUCCESSFULLY! ID:", data.id);
//       }
//     } else {
//       console.log("ℹ️ No *new* unique leads to notify about.");
//     }
//   } catch (err) {
