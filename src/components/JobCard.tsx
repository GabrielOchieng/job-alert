...
export default function JobCard({ job }: { job: Job }) {
  ...
  const [isTailorOpen, setIsTailorOpen] = useState(false);
  const [activeAction, setActiveAction] = useState<
    "analyzing" | "pitching" | "status" | "deleting" | null
  >(null);
  ...
  const deleteMutation = useMutation({
    ...
    onMutate: () => setActiveAction("deleting"),
    onSettled: () => setActiveAction(null),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["jobs"] });
      queryClient.invalidateQueries({ queryKey: ["job-counts"] });
      toast.success("Signal purged from database");
    },
    onError: () => {
      toast.error("Failed to delete signal. Check RLS policies.");
    },
  });
      ...
      <CardFooter className="flex flex-wrap items-center justify-between gap-2 pt-2 bg-primary/5 py-3">
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            ...
            size="sm"
            onClick={() => {
              if (confirm("Are you sure you want to purge this signal?")) {
                deleteMutation.mutate();
              }
            }}
            disabled={!!activeAction}
            className="h-8 hover:bg-destructive/10 text-destructive text-xs font-bold uppercase tracking-tighter"
          >
